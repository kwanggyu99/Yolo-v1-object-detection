...
class yoloLoss(Module):
    ...
    def forward(self, pred, target):
        ...
        # 좌표값의 index를 갖음
        obj_mask = (target_boxes[..., 4] > 0).byte()    # target에 대한 confidence score
        sig_mask = obj_mask[..., 1].bool()              # 0보다 큰 곳에 True로 설정
        index = torch.where(sig_mask == True)           # grid 중 1이라고 marking된 cell의 index값
        ...
        #각 loss값의 합산
        loss = obj_loss + (self.lambda_noobj * noobj_loss) + (self.lambda_coord * xy_loss) + (self.lambda_coord * wh_loss) + class_loss 
