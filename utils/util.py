...
# 학습이 다 끝나고 inference 과정에서 사용 (tensor구조 -> img boxes, each class)
def decoder(prediction):
    ...
    prediction = prediction.data.squeeze()  # (1x14x14x30) --> 14x14x30
    # 2개 박스의 각 confidence score
    contain1 = prediction[:, :, 4].unsqueeze(2)     # (14x14) --> [14,14,1]
    contain2 = prediction[:, :, 9].unsqueeze(2)     # (14x14) --> [14,14,1]
        ...
        for j in range(grid_num):
            for b in range(2):
                if mask[i, j, b] == 1:
                    ...
                    # (0~1)정규화로 저장했던 size를 원상복구
                    xy = torch.FloatTensor([j, i]) * cell_size
                    box[:2] = box[:2] * cell_size + xy.to(device)
                    box_xy = torch.FloatTensor(box.size())
                    box_xy[:2] = box[:2] - 0.5 * box[2:] #bbox의 좌측 상단 좌표
                    box_xy[2:] = box[:2] + 0.5 * box[2:] #bbox의 우측 하단 좌표
                    #bbox의 분류값과 confidence score 계산
                    max_prob, cls_index = torch.max(prediction[i, j, 10:], 0) #11~30 tensor중에서 (최대 확률값, 해당 idx)
                    if float((contain_prob * max_prob)[0]) > 0.1: # class confidence값 
