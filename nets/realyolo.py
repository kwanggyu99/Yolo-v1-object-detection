...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, anchors=[(1.3221, 1.73145), (3.19275, 4.00944), (5.05587, 8.09892), (9.47112, 4.84053),(11.2364, 10.0071)]):
        ...
        # concat된 블럭을 output shape(13, 13, 125) 형태로 바꿔주는 마지막 작업
        self.conv_end1 = nn.Sequential(nn.Conv2d(1024+512, 1024, 3, 1, 1, bias=False),  nn.LeakyReLU(0.1, inplace=True))

        self.conv_end2 = nn.Conv2d(1024, len(self.anchors) * (1+ 4 + 20), 1, 1, 0, bias=False)
    ...
    def forward(self, x):
        ...
        concat = torch.cat([p4, p5], dim=1) # (1024+512, 14, 14) --> (1536, 13, 13)
        
        x = self.conv_end1(concat)
        x = self.conv_end2(x)               # (batch, 5*25, 13, 13)

        #x = x.permute(0, 2, 3, 1)           # (batch, 13, 13, 5*25)
        # output = {
